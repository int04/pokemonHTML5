...
function leven_verandering2(life_new,wie,life_max){
  //Calculate Green Div, And Set life_max
  var green_new = Math.round(life_new/life_max*100);

  //Request Current Green div
  var green_old = parseInt(document.getElementById(wie + '_life2').style.width);
  ...
  //Life is going down
  if(green_old < green_new){
    oplopend2(green_old,green_new,wie,life_old,life_new,life_max);
  }
  //Life is going up
  else if(green_old > green_new){
    aflopend2(green_old,green_new,wie,life_old,life_new,life_max);
  }
  else if (green_old == green_new){
    clearTimeout(timer);
  }
...
// Life down function
function oplopend2(green_old,green_new,wie,life,life_new,life_max){
  //Life old is different than life new
  if(green_old != green_new){
    
    //Green div 1px bigger
    green_old += 1;
    document.getElementById(wie + '_life2').style.width = green_old + '%';

    //Do it agian
    timer = setTimeout('leven_verandering2( '+ life_new + ', "'+ wie+ '", ' + life_max + ');', 5);
  }
  //life old is same as life new
  else{
    //Stop timer
...
function aflopend2(green_old,green_new,wie,life,life_new,life_max){
  //Life old is different than life new
  if(green_old != green_new){
...
function exp_change2(exp_new,exp_max){

  //Calculate Blue Div, And Set life_max
  var blue_new = Math.round(exp_new/exp_max*100);

  //Request Current Blue div
  var blue_old = parseInt(document.getElementById('pokemon_exp2').style.width);
...
// Exp down function
function oplopend_exp2(blue_old,blue_new,exp_old,exp_new,exp_max){
  //Exp old is different than Exp new
  if(blue_old != blue_new){
    
    //Blue div 1px bigger
    blue_old += 1;
    document.getElementById('pokemon_exp2').style.width = blue_old + '%';

    //Do it agian
    timer = setTimeout('exp_change2( '+ exp_new + ', ' + exp_max + ');', 5);
  }
  //life old is same as Exp new
  else{
    //Stop timer
